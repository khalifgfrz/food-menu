...
function Menu() {
  ...
  return (
    <main className="menu">
      ...
      {numFoods > 0 ? (
        <>
          <p>Aneka makanan Indonesia yang disajikan oleh warteg mang udin sebagai pemenuhan makanan kesehatan yang diperlukan dalam kehidupan sehari-hari.</p>
          <ul className="foods">
            {data.map((food) => (
...
function FooterClosedHour({ jamBuka, jamTutup }) {
  return (
    <footer className="footer">
      <p>
        Maaf gan masih tutup. Coba datang lagi sekitar jam {jamBuka}-{jamTutup}
      </p>
    </footer>
  );
}
